...
class BorrowController extends Controller
{
    public function index()
    {
        ...
        $activities = Borrow::with('roomItem')
            ...
            ->groupBy(function($borrow) {
                ...
                // Ensure borrow_date is properly formatted
                try {
                    if (empty($borrow->borrow_date)) {
                        $dateStr = Carbon::now()->format('Y-m-d H:i');
                    } elseif ($borrow->borrow_date instanceof Carbon) {
                        $dateStr = $borrow->borrow_date->format('Y-m-d H:i');
                    } else {
                        $dateStr = Carbon::parse($borrow->borrow_date)->format('Y-m-d H:i');
                    }
                } catch (\Exception $e) {
                    $dateStr = Carbon::now()->format('Y-m-d H:i');
                }
                return ($borrow->borrower_name ?? '') . '|' . $dateStr . '|' . ($borrow->latitude ?? '') . '|' . ($borrow->longitude ?? '');
            })
            ->map(function($group) {
    ...
    public function returnItem($id)
    {
        ...
        if ($returnFullSet) {
            $fullSetBorrows = Borrow::with('roomItem')
                ...
                ->whereDate('borrow_date', $borrow->borrow_date instanceof Carbon ? $borrow->borrow_date->format('Y-m-d') : Carbon::parse($borrow->borrow_date)->format('Y-m-d'))
                ->get();
    ...
    public function returnBulk(Request $request)
    {
        ...
        foreach ($request->items as $borrowId => $itemData) {
            ...
            if ($itemData['status'] === 'Unusable') {
                $noteText = 'Item: ' . $item->device_category . ' (' . $item->serial_number . ')' . "\n";
                $noteText .= 'Room: ' . $item->room_title . "\n";
                $noteText .= 'Returned as Unusable' . "\n";
                if (!empty($itemData['reason'])) {
                    $noteText .= 'Reason: ' . $itemData['reason'];
                } else {
                    $noteText .= 'Reason: Not specified';
                }
                ...
                MaintenanceNote::updateOrCreate(
                    ...
                    [
                        'note' => $noteText,
                        'fullset_id' => $item->full_set_id ?? null,
                    ]
                );
                $unusableCount++;
            }

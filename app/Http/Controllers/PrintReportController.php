...
class PrintReportController extends Controller
{
    public function index(Request $request)
    {
        try {
            ...
            $roomItems = $roomItems ?? collect([]);

            // Return both datasets to the view
            return view('print-report', compact('borrowedItems', 'roomItems', 'filter'));
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error('PrintReportController error: ' . $e->getMessage());
            return redirect()->back()->withErrors(['error' => 'An error occurred while loading the report.']);
        }
    }
}

                            ...
                            <div class="pc-group" id="pc-{{ Str::slug($roomTitle) }}-{{ $pcId }}" data-container="{{ Str::slug($roomTitle) }}-{{ $pcId }}">
                                <div class="pc-header" onclick="togglePCGroup('{{ Str::slug($roomTitle) }}-{{ $pcId }}')">
                                    ...
                                    <div class="pc-stats">
                                        ...
                                        <button class="room-delete-btn" style="margin-left:8px; background:#dc3545;" onclick="deletePCGroup('{{ Str::slug($roomTitle) }}-{{ $pcId }}', '{{ $pcTitle }}', event)"><i class="fas fa-trash"></i> Delete PC</button>
                                        <div class="toggle-icon">▶</div>
                                    </div>
                                </div>
                                    ...
                                    <div class="table-container">
                                        <table class="maintenance-table">
                                            <thead>
                                                <tr>
                                                    <th>
                                                        <input type="checkbox" class="select-all-checkbox" data-container-id="{{ Str::slug($roomTitle) }}-{{ $pcId }}" onchange="toggleSelectAll(this)" onclick="event.stopPropagation()">
                                                    </th>
                                                    <th>Photo</th>
                                                    <th>Barcode</th>
                                                    <th>Category</th>
                            ...
                            <div class="pc-group" id="pc-{{ Str::slug($roomTitle) }}-solo" data-container="{{ Str::slug($roomTitle) }}-solo">
                                <div class="pc-header" onclick="togglePCGroup('{{ Str::slug($roomTitle) }}-solo')">
                                    ...
                                    <div class="pc-stats">
                                        ...
                                        <button class="room-delete-btn" style="margin-left:8px; background:#dc3545;" onclick="deletePCGroup('{{ Str::slug($roomTitle) }}-solo', 'Solo Items', event)"><i class="fas fa-trash"></i> Delete All</button>
                                        <div class="toggle-icon">▶</div>
                                    </div>
                                </div>
                                    ...
                                    <div class="table-container">
                                        <table class="maintenance-table">
                                            <thead>
                                                <tr>
                                                    <th>
                                                        <input type="checkbox" class="select-all-checkbox" data-container-id="{{ Str::slug($roomTitle) }}-solo" onchange="toggleSelectAll(this)" onclick="event.stopPropagation()">
                                                    </th>
                                                    <th>Photo</th>
                                                    <th>Barcode</th>
                                                    <th>Category</th>
...
<script>
    ...
    function updateBulkActions() {
        ...
        document.querySelectorAll('.room-group').forEach(roomGroup => {
            ...
            const roomDeleteBtn = roomDeleteContainer ? roomDeleteContainer.querySelector('.room-delete-btn') : null;
        ...
        // Update select-all checkboxes for each PC group
        document.querySelectorAll('.pc-group').forEach(pcGroup => {
            const allCheckboxes = pcGroup.querySelectorAll('.item-checkbox');
            const checkedCheckboxes = pcGroup.querySelectorAll('.item-checkbox:checked');
            const selectAllCheckbox = pcGroup.querySelector('.select-all-checkbox');
    ...
    function toggleSelectAll(checkboxElement) {
        // Handle both string (legacy) and element parameters
        let checkbox = checkboxElement;
        if (typeof checkboxElement === 'string') {
            // Legacy support: find checkbox by container ID
            const containerId = checkboxElement;
            const container = document.querySelector(`[data-container="${containerId}"]`) ||
                            document.querySelector(`#pc-${containerId}`) ||
                            document.querySelector(`#fullset-${containerId}`);
            if (container) {
                checkbox = container.querySelector('.select-all-checkbox');
            }
            if (!checkbox) return;
        }
        
        // Ensure we have a valid checkbox element
        ...
        // Find the PC group container - try multiple methods
        let pcGroup = checkbox.closest('.pc-group');
        
        // If not found, try using data-container-id
        if (!pcGroup) {
            const containerId = checkbox.getAttribute('data-container-id');
            if (containerId) {
                pcGroup = document.querySelector(`[data-container="${containerId}"]`) ||
                         document.querySelector(`#pc-${containerId}`);
            }
        }
        
        // If still not found, traverse up manually
        if (!pcGroup) {
            let element = checkbox;
            while (element && element !== document.body) {
                if (element.classList && element.classList.contains('pc-group')) {
                    pcGroup = element;
                    break;
                }
        ...
        if (!pcGroup) {
            console.error('Could not find PC group container');
            return;
        }
        
        // Ensure PC group is expanded so checkboxes are accessible
        if (!pcGroup.classList.contains('expanded')) {
            pcGroup.classList.add('expanded');
        }
        
        // Find all item checkboxes in this PC group
        // querySelectorAll works even on hidden elements, but let's be thorough
        let checkboxes = [];
        
        // First, try to find in table tbody (most specific)
        const table = pcGroup.querySelector('table');
        if (table) {
            const tbody = table.querySelector('tbody');
            if (tbody) {
                checkboxes = tbody.querySelectorAll('.item-checkbox');
            } else {
                checkboxes = table.querySelectorAll('.item-checkbox');
            }
        }
        
        // If no checkboxes found, search pc-content
        if (checkboxes.length === 0) {
            const pcContent = pcGroup.querySelector('.pc-content');
            if (pcContent) {
                checkboxes = pcContent.querySelectorAll('.item-checkbox');
            }
        }
        
        // Last resort: search entire pc-group
        if (checkboxes.length === 0) {
            checkboxes = pcGroup.querySelectorAll('.item-checkbox');
            // Exclude the select-all checkbox itself
            checkboxes = Array.from(checkboxes).filter(cb => !cb.classList.contains('select-all-checkbox'));
        }
        
        if (checkboxes.length === 0) {
            console.warn('No items found in this PC group. PC Group ID:', pcGroup.id);
            return;
        }
    ...
    // Event delegation for select-all checkboxes (works even if content is dynamically loaded)
    document.addEventListener('change', function(e) {
        if (e.target && e.target.classList && e.target.classList.contains('select-all-checkbox')) {
            e.stopPropagation();
    ...
    function deletePCGroup(containerId, pcTitle, event) {
        // Stop event propagation to prevent PC group toggle
        if (event) {
            event.stopPropagation();
        }
        
        // Find the PC group container
        let pcGroup = document.querySelector(`[data-container="${containerId}"]`) ||
                     document.querySelector(`#pc-${containerId}`);
        ...
        if (!pcGroup) {
            ...
            if (parts.length >= 2) {
                ...
                const pcId = parts.slice(1).join('-');
                pcGroup = document.querySelector(`#pc-${roomSlug}-${pcId}`);
            }
        }
        ...
        if (!pcGroup) {
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Could not find PC group container.'
            });
            return;
        }
        
        // Ensure PC group is expanded to find all checkboxes
        if (!pcGroup.classList.contains('expanded')) {
            pcGroup.classList.add('expanded');
        }
        ...
        if (checkboxes.length === 0) {
            Swal.fire({
                icon: 'info',
                title: 'No Items',
                text: 'This PC group has no items to delete.'
            });
            return;
        }
        
        // Get all item IDs
        const itemIds = Array.from(checkboxes).map(checkbox => checkbox.dataset.itemId).filter(id => id);
        
        if (itemIds.length === 0) {
            Swal.fire({
                icon: 'info',
                title: 'No Items',
                text: 'No valid items found to delete.'
            });
            return;
        }
        ...
        Swal.fire({
            title: 'Delete PC Group?',
            html: `Are you sure you want to delete <strong>${pcTitle}</strong>?<br><br>This will delete <strong>${itemIds.length} item(s)</strong> permanently. This action cannot be undone!`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
            cancelButtonColor: '#6c757d',
            ...
            if (result.isConfirmed) {
                // Delete all items using the existing bulk delete function
                bulkDeleteItems(itemIds);
                
                // Optionally remove the PC group container if it becomes empty
                // The items will be removed from DOM by bulkDeleteItems
                // We can check if the PC group is now empty and hide/remove it
                setTimeout(() => {
                    const remainingCheckboxes = pcGroup.querySelectorAll('.item-checkbox');
                    const remainingItems = Array.from(remainingCheckboxes).filter(cb => !cb.classList.contains('select-all-checkbox'));
                    if (remainingItems.length === 0) {
                        // Fade out and remove the PC group
                        pcGroup.style.transition = 'opacity 0.3s ease';
                        pcGroup.style.opacity = '0';
                        setTimeout(() => {
                            pcGroup.remove();
                            // Update room stats if needed
                            updateBulkActions();
                        }, 300);
                    } else {
                        // Update bulk actions to reflect changes
                        updateBulkActions();
                    }
                }, 500);
            }
